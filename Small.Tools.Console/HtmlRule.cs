                        ...
                        new Thread(new ThreadStart(delegate
                        {
                            ...
                            try
                            {
                                using (StreamWriter writer = new StreamWriter(ctx.Response.OutputStream))
                                {
                                    ...
                                    {
                                        System.Console.WriteLine("{0}> 删除代理{1}", DateTime.Now.ToString("s"), ipp);
                                    ...
                                    else
                                    {
                                        ...
                                        while (true)
                                        {
                                            ...
                                            if (que.First().Key > 0)
                                            {
                                                ...
                                                {
                                                    System.Console.WriteLine("{0}> 直接输出{1}", DateTime.Now.ToString("s"), que.First().Value.Id);
                                                    // 输出http响应代码
                                                ...
                                                else
                                                {
                                                    ...
                                                    {
                                                        System.Console.WriteLine("{0}> 验证输出{1}", DateTime.Now.ToString("s"), que.First().Value.Id);
                                                        // 输出http响应代码
                                                        writer.WriteLine(que.First().Value.Id);
                                                        // 退出本次请求
                                                        break;
                            ...
                            catch (Exception ex)
                            {
                                try
                                {
                                    System.Console.WriteLine("{0}> 接口異常：{1}", DateTime.Now.ToString("s"), ex.Message);
                ...
                using (WebClient web = new WebClient())
                {
                    try
                    {
                        ...
                        using (StreamReader sr = new StreamReader(Resp.GetResponseStream(), bin))
                        {
                            ...
                            else { System.Console.WriteLine("{0}> [{2}]自动验证失败{1}", DateTime.Now.ToString("s"), proxy.Id, name); }
                        }
                    }
                    catch (Exception ex)
                    { }
